...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    # 将 mask 加入到缩放的张量上, mask的地方给一个最小值

    if mask is not None:
        # 这边返回的是mask 是 True的片段，要专程1，0的
        mask_s = tf.cast(tf.equal(mask, False), 'float32')
        mask_s = tf.expand_dims(mask_s, 1)
        mask_s = tf.expand_dims(mask_s, 1)
        scaled_attention_logits += (mask_s * -1e9)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def compute_nask(self, inputs, mask=None):
        return None
